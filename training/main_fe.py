...
from feature_engineering_losses import FeatureEngineeringLoss, PCAProjector, pca_attribute_ce, predict_x0_from_noise
...
def parse_args(pretrained_model_name_or_path, emotion, train_data_dir, learnable_property, max_train_steps,
    ...
    parser.add_argument("--fe_color_rate", type=float, default=fe_color_rate,
                        help="Weight for RGB soft color histogram + color moments loss.")
    parser.add_argument("--fe_hog_rate", type=float, default=fe_hog_rate,
    ...
    parser.add_argument("--fe_pca_mix", type=float, default=fe_pca_mix,
                        help="Mixing ratio for PCA-whitened attribute CE loss.")
    parser.add_argument("--fe_pca_temperature", type=float, default=fe_pca_temperature,
                        help="Temperature for PCA attribute contrastive logits.")
    parser.add_argument("--fe_decode_every", type=int, default=fe_decode_every,
                        help="Compute decoded feature losses every N global steps.")
    parser.add_argument("--fe_warmup_steps", type=int, default=fe_warmup_steps,
                        help="Start decoded feature losses after this many global steps.")
...
def main(args):
    ...
    feature_regularizer = FeatureEngineeringLoss(
    ...
    pca_projector = None
    if args.fe_pca_dim is not None and args.fe_pca_dim > 0:
        # PCA-whitening is fitted once on all attribute CLIP text features.
        pca_projector = PCAProjector(args.fe_pca_dim).to(total_attr_embed.device)
        pca_projector.fit(total_attr_embed.float())
            ...
            with accelerator.accumulate(encoder):
                with accelerator.accumulate(mapper):
                    ...
                    if batch["attribute"][0].lower() in total_attr:
                        ...
                        loss_attr_pca = torch.zeros_like(loss_attr)
                    ...
                    loss_dct = torch.zeros_like(loss_fe)
                    if (
                        args.fe_use
                        and global_step >= args.fe_warmup_steps
                        and args.fe_decode_every > 0
                        and global_step % args.fe_decode_every == 0
                    ):
                        pred_x0 = predict_x0_from_noise(noise_scheduler, noisy_latents, timesteps, model_pred)
                        pred_img = vae.decode((pred_x0 / vae.config.scaling_factor).to(dtype=weight_dtype)).sample
                        target_img = batch["pixel_values"].to(device=pred_img.device, dtype=pred_img.dtype)
                        loss_fe, fe_logs = feature_regularizer(pred_img, target_img)
                        loss_color = fe_logs["loss_color"].to(loss_fe.device)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    for tracker in accelerator.trackers:
                        ...
                        tracker.writer.add_scalar("loss_fe", loss_fe, global_step)
                        tracker.writer.add_scalar("loss_color_hist_moment", loss_color, global_step)
                        tracker.writer.add_scalar("loss_hog", loss_hog, global_step)
                        tracker.writer.add_scalar("loss_dct", loss_dct, global_step)
                        tracker.writer.add_scalar("loss_attr_pca", loss_attr_pca, global_step)
...
if __name__ == "__main__":
    ...
    file_name = 'config/my_config.yaml'
